...
import { collection, getDocs, deleteDoc, doc, getDoc, setDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
async function carregarListaDeItens() {
    ...
    try {
        ...
        listarCadastrosDiv.innerHTML = '<p> Erro ao carregar a lista de funcionários... </p>';
    }
}
...
function renderizarListaDeItens(cadastros) {
    ...
    if (cadastros.length === 0) {
        listarCadastrosDiv.innerHTML = '<p> Nenhum Funcionário cadastrado...</p> ';
        return;
    }
    for (let cadastro of cadastros) {
        ...
        cadastroDiv.innerHTML = `
            <strong> Produto: </strong> ${cadastro.produto} <br>
            <strong> Quantidade: </strong> ${cadastro.quantidade} <br>
            <button class="btn-Excluir" data-id="${cadastro.id}"> Excluir </button>
            <button class="btn-Editar" data-id="${cadastro.id}"> Editar </button>
        `;
